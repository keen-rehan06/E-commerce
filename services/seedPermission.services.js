...
import { ADMIN, CUSTOMER, DELIVERY_PARTNER, PERMISSIONS, VENDOR, SUPPORT } from "./permissions.services.js";
...
export const seedPermission = async (req,res) => {
    ...
    const getPermissionId = (permissionName) => {
        return createdPermission.filter((permission) => permissionName.include(permission.name)).map((permission) => permission._id);
    }

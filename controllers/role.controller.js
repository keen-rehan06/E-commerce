...
export const assignRole = async (req, res) => {
  try {
    ...
    if(admin.id.toString() === user._id.toString()) return res.status(401).send({message:"You can't change your own role", success: true});

    if(!allowedRoles.includes(role)) return res.status(401).send({message:"You are not allowed to assign this role.",success:false});
    user.role = role;
    await user.save();
